...
import { sendRegistrationOTP, verifyRegistrationOTP, registerSociety, searchIndianColleges } from "../../services/registrationApi";
...
export default function SocietyRegister({ onClose }) {
  ...
  const validateStep1 = () => {
    if (!societyName.trim()) { toast.error("Society name is required."); return false; }
  ...
  const handleRegister = async () => {
    ...
    try {
      await registerSociety({
        societyName, collegeName, logoUrl,
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center py-10 px-4 text-center gap-4">
        ...
        <h3 className="text-xl font-bold text-white">Society Registered!</h3>
        <p className="text-gray-400 text-sm max-w-xs">
          Your society has been registered successfully. Our team will review your application and get in touch.
        </p>
          ...
          {step === 1 && (
            <motion.div key="step1" custom={dir} {...slide} className="space-y-4">
              <div>
                ...
                <input className={inputCls} value={societyName} onChange={e => setSocietyName(e.target.value)}
                  placeholder="e.g. GFG Student Chapter" />
              </div>
              <SearchableDropdown
                label="College Name" id="soc-college" required
                value={collegeName} onChange={setCollegeName}
                fetchOptions={fetchColleges}
                placeholder="Search your college…"
              />
              <LogoUpload value={logoUrl} onChange={setLogoUrl} label="Society Logo (optional)" />
            ...
            <motion.div key="step2" custom={dir} {...slide} className="space-y-4">
              <div>
                ...
                <input type="email" className={inputCls} value={email} onChange={e => setEmail(e.target.value)}
                  placeholder="society@example.com" required />
              </div>
              <PasswordField id="soc-password" label="Create Password" value={password} onChange={setPassword} />
              <PasswordField id="soc-confirm" label="Confirm Password" value={confirmPassword} onChange={setConfirmPassword} placeholder="Re-enter password" />
