...
export default function UniversityRegister({ onClose }) {
  ...
  const handleRegister = async () => {
    ...
    try {
      await registerUniversity({
        universityName, state, city, pinCode, address, logoUrl,
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center py-10 px-4 text-center gap-4">
        ...
        <h3 className="text-xl font-bold text-white">University Registered!</h3>
        <p className="text-gray-400 text-sm max-w-xs">
          Your university has been registered successfully. Our team will review your application and get in touch.
        </p>
        ...
        <AnimatePresence mode="wait" custom={dir}>
          {step === 1 && (
            <motion.div key="step1" custom={dir} {...slide} className="space-y-4">
              ...
              <LogoUpload value={logoUrl} onChange={setLogoUrl} label="University Logo (optional)" />
            ...
            <motion.div key="step2" custom={dir} {...slide} className="space-y-4">
              <div>
                ...
                <input type="email" className={inputCls} value={email} onChange={e => setEmail(e.target.value)}
                  placeholder="university@example.com" required />
              </div>
              <PasswordField id="uni-password" label="Create Password" value={password} onChange={setPassword} />
              <PasswordField id="uni-confirm" label="Confirm Password" value={confirmPassword} onChange={setConfirmPassword} placeholder="Re-enter password" />
              <div className="flex gap-2 pt-1">
                <button type="button" onClick={() => go(1)}
                  className="py-2.5 px-4 rounded-xl border border-white/10 text-gray-400 hover:bg-white/5 transition-colors text-sm">
                  ← Back
                </button>
                <button onClick={handleSendOTP} disabled={loading}
                  className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold disabled:opacity-50 transition-opacity">
              ...
              <div className="flex gap-2">
                {!otpVerified ? (
                  ...
                  <button onClick={handleRegister} disabled={loading}
                    className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold disabled:opacity-50 hover:opacity-90 transition-opacity">
